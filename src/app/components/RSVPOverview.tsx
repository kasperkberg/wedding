...
export function RSVPOverview() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Detailed List */}
      <div className="bg-white rounded-xl shadow-lg p-6">
        <h3 className="text-xl font-bold text-gray-900 mb-4">Alle RSVP'er</h3>
        <div className="space-y-4">
          {rsvps.map((rsvp) => (
            <div key={rsvp.id} className="border border-gray-200 rounded-lg p-4">
              ...
              {rsvp.attending && (
                <div className="mb-3">
                  <h5 className="font-medium text-sm text-gray-700 mb-1">Kostinformation:</h5>
                  <div className="text-sm text-gray-600 space-y-1">
                    {rsvp.allergies && (
                      <p><strong>Allergier:</strong> {rsvp.allergies}</p>
                    )}
                    {rsvp.foodPreferences && (
                      <p><strong>Madpræferencer:</strong> {rsvp.foodPreferences}</p>
                    )}
                      ...
                      <div key={guest.id} className="bg-gray-50 p-3 rounded">
                        ...
                        {guest.attending && (
                          <div className="text-sm text-gray-600 mt-2 space-y-1">
                            {guest.allergies && (
                              <p><strong>Allergier:</strong> {guest.allergies}</p>
                            )}
                            {guest.foodPreferences && (
                              <p><strong>Madpræferencer:</strong> {guest.foodPreferences}</p>
                            )}
